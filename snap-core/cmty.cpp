...
// OutDegH and OrigEdges stores node degrees and number of edges in the original graph
        double
        _GirvanNewmanGetModularity(const PUNGraph &G, const TIntH &OutDegH, const int &OrigEdges, TCnComV &CnComV) {
        ...
        bool inComp(PNGraph &g1, PNGraph &Graph, TIntH &inCompCount, int id, int neigh) {
            ...
            if (g1->IsNode(id) && g1->IsNode(neigh)) {
                ...
                if (inCompN < neighDeg && inComp < deg &&
                    (!g1->IsNode(neigh) || Graph->GetNI(neigh).GetDeg() - neighDeg == 0)) {
        ...
        double InfomapOnlineIncrement(PUNGraph &Graph, int n1, int n2, TIntFltH &PAlpha, double &SumPAlphaLogPAlpha,
                                      TIntFltH &Qi, TIntH &Module, int &Br) {
...
// See: Rosvall M., Bergstrom C. T., Maps of random walks on complex networks reveal community structure, Proc. Natl. Acad. Sci. USA 105, 1118-1123 (2008)
    ...
    double InfomapOnline(PUNGraph &Graph, int n1, int n2, TIntFltH &PAlpha, double &SumPAlphaLogPAlpha, TIntFltH &Qi,
                         TIntH &Module, int &Br, TCnComV &CmtyV) {

        double MinCodeLength = TSnapDetail::InfomapOnlineIncrement(Graph, n1, n2, PAlpha, SumPAlphaLogPAlpha, Qi,
                                                                   Module, Br);
    ...
    void CmtyEvolutionFileBatchV(TStr InFNm, TIntIntVH &sizesContV, TIntIntVH &cContV, TIntIntVH &edges, double alpha,
                                 double beta, int CmtyAlg) {
        TIntIntHH sizesCont;
        TIntIntHH cCont;
    ...
    void CmtyEvolutionFileBatch(TStr InFNm, TIntIntHH &sizesCont, TIntIntHH &cCont, TIntIntVH &edges, double alpha,
                                double beta, int CmtyAlg) {
    ...
    void CmtyEvolutionJson(TStr &Json, TIntIntVH &sizesContV, TIntIntVH &cContV, TIntIntVH &edges) {
        ...
        for (THashKeyDatI <TInt, TIntV> it = edges.BegI(); !it.IsEnd(); it++) {
            ...
            // writing to string
            Json.InsStr(Json.Len(), "{\"n1\":");
            Json.InsStr(Json.Len(), n1.GetStr());
            Json.InsStr(Json.Len(), ", \"n2\":");
            Json.InsStr(Json.Len(), n2.GetStr());
            Json.InsStr(Json.Len(), ", \"w\":");
            Json.InsStr(Json.Len(), w.GetStr());
            Json.InsStr(Json.Len(), ", \"t0\":");
            Json.InsStr(Json.Len(), t0.GetStr());
            Json.InsStr(Json.Len(), ", \"t1\":");
            Json.InsStr(Json.Len(), t1.GetStr());
            Json.InsStr(Json.Len(), " }\n");
            br++;
        }
        ...
        for (int i = 0; i < sizesContV[0].Len(); i++) {
            for (THashKeyDatI <TInt, TIntV> it = sizesContV.BegI(); !it.IsEnd(); it++) {
                ...
                if (size > 0) {
                    ...
                    TInt size = it.GetDat()[i];
                    Json.InsStr(Json.Len(), "{\"id\":");
                    Json.InsStr(Json.Len(), id.GetStr());
                    Json.InsStr(Json.Len(), ", \"size\":");
                    Json.InsStr(Json.Len(), size.GetStr());
                    Json.InsStr(Json.Len(), ", \"t\":");
                    Json.InsStr(Json.Len(), j.GetStr());
                    Json.InsStr(Json.Len(), " }\n");
        ...
        if (collapse) {
            for (TNGraph::TNodeI NI = gFinal->BegNI(); NI < gFinal->EndNI(); NI++) {
                if (NI.GetInDeg() == 1 && NI.GetOutDeg() == 1)
                    if (gFinal->GetNI(NI.GetInNId(0)).GetOutDeg() == 1 &&
                        gFinal->GetNI(NI.GetOutNId(0)).GetInDeg() == 1) {
    ...
    void ReebRefine(PNGraph &Graph, TIntH &t, int e, PNGraph &gFinal, TIntH &tFinal, bool collapse) {
        ...
        for (int i = 0; i < timePoints.Len(); i++) {
            ...
            for (TNGraph::TNodeI NI = g1->BegNI(); NI < g1->EndNI(); NI++) {
                ...
                if (focusTimePoint + (e / 2) ==
                    t.GetDat(NI.GetId())) { // if its on the right edge only in degree is observed
                    RO = FO = 0;
                }
                if (focusTimePoint - (e / 2) ==
                    t.GetDat(NI.GetId())) { // if its on the left edge only out degree is observed
                    RI = FI = 0;
                }
            ...
            if (cms0.Len() > 0 && cms1.Len() > 0) {
                for (int i = 0; i < cms0.Len(); i++) {
                    for (int j = 0; j < cms1.Len(); j++) {
                        ...
                        if (TSnapDetail::vectorIntersect(ids0, ids1) == smaller ||
                            (smaller > 2 && TSnapDetail::vectorIntersect(ids0, ids1) == (smaller - 1))) {
                            if (!gFinal->IsNode(cms0[i])) {
                                gFinal->AddNode(cms0[i]);
                                tFinal.AddDat(cms0[i], focusTimePoint);
                            }
        ...
        public:
            ...
            TFltIntIntTr FindMxQEdge() {
                while (true) {
                    ...
                    if (TopQ.Val1 != CmtyQH.GetDat(TopQ.Val2).GetMxQ() &&
                        TopQ.Val1 != CmtyQH.GetDat(TopQ.Val3).GetMxQ()) { continue; }
                    return TopQ;
                }
            ...
            bool MergeBestQ() {
                ...
                TCmtyDat &DatJ = CmtyQH.GetDat(J);
                {
                    ...
                    for (int i = -1; DatJ.NIdQH.FNextKeyId(i);) {
                        ...
                        if (DatI.NIdQH.IsKey(K)) {
                            NewQ = NewQ + DatI.NIdQH.GetDat(K);
                            DatK.DelLink(I);
